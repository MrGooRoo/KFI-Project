#!/usr/bin/env python3
"""
KFI Card Generator v4.4 — Добавлены описания блоков, нормы и объём облигаций
"""
...
class CardGenerator:
    ...
    def get_or_calculate(self, emitter: Dict) -> Dict:
        if self.kfi_calculator:
            ...
            try:
                return self.kfi_calculator.calculate_kfi(emitter, mock_financials, "2024-Q4")
            except Exception as e:
                print(f"  Предупреждение при расчёте {emitter.get('emitter_id')}: {e}")
        return None

    def prepare_context(self, emitter: Dict, calculation: Dict = None) -> Dict:
        if not calculation:
            calculation = self.get_or_calculate(emitter) or {
                "kfi_final": 68.0, "category": "Стабильный",
                "block_scores": {"block_a": 65, "block_b": 70, "block_c": 58, "block_d": 62, "block_e": 81, "block_f": 77},
        ...
        # Все действующие облигации (по дате погашения + статус)
        active_bonds = []
        ...
        for b in emitter.get("bonds", []):
            ...
            if maturity_str:
                try:
                    maturity_date = datetime.strptime(maturity_str.split("T")[0], "%Y-%m-%d").date()
        ...
        return {
            ...
            "category_color": self.category_colors.get(calculation.get("category"), "#64748b"),
    ...
    def generate_card(self, emitter_id: str):
        ...
        with open(html_path, "w", encoding="utf-8") as f:
            f.write(html)
        print(f"✅ HTML-карточка для {emitter_id} сохранена")
    ...
    def generate_all(self):
        ...
        print(f"\n🎉 Все карточки (HTML + PNG) сохранены в: {self.output_dir}")


if __name__ == "__main__":
    print("🚀 Запуск KFI Card Generator v4.4 (описания блоков + нормы + объём облигаций)")
